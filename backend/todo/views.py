...
def FoodApi(request: HttpRequest, id=0):
    if request.method == 'GET':
        ...
        foods_serializers = FoodsSerializer(
            food, data=food_data).update(food, food_data)
    ...
    if request.method == 'GET':
        if id == 0:
            ...
            reservation = Reservation.objects.filter(
                rtime__date__range=(begin, end)).order_by('-id')
            reservation_serializers = ReservationSerializer(
                reservation, many=True)
            return JsonResponse(reservation_serializers.data, safe=False)
        else:
            reservation = Reservation.objects.get(id=id)
            reservation_serializer = ReservationSerializer(
                reservation, many=False)
        ...
        reservations_serializers = ReservationSerializer(
            reservation, data=reservation_data)
        if reservations_serializers.is_valid():
            reservations_serializers.save()
            return JsonResponse("Update Successfully", safe=False)
        return JsonResponse("Failed to Update")
    ...
    if request.method == 'GET':
        if id == 0:
            ...
            order = Order.objects.filter(
                time__date__range=(begin, end)).order_by('-id')
...
def summaryApi(request):
    if request.method == 'GET':
        sm = Order.objects.aggregate(
            sum_total=Sum('total'), num_order=Count('id'))
...
def graphApi(request):
    if request.method == 'GET':
        if 'year' not in request.GET:
            rq = Order.objects.filter(time__date__range=(datetime.today().date(
            )-timedelta(7), datetime.today().date())).values('time__date').order_by('time__date').annotate(count=Count('id'))
            return JsonResponse(list(rq), safe=False)
        elif 'month' not in request.GET:
            rq = Order.objects.filter(time__year=request.GET['year']).values(
                'time__month').order_by('time__month').annotate(count=Count('id'), sum=Sum('total'))
            return JsonResponse(list(rq), safe=False)
        else:
            rq = Order.objects.filter(time__year=request.GET['year'], time__month=request.GET['month']).values(
                'time__date').order_by('time__date').annotate(count=Count('id'), sum=Sum('total'))
            return JsonResponse(list(rq), safe=False)
